                            ...
                            <div class="xs-single-pricing <?php if ( $table_value['xs_featured_table'] == 'yes' ) echo 'active'; ?>">
                                <div class="pricing-header">
                                    ...
                                    <?php if (!empty($table_value['table_title'])) : ?>
                                        <h4 class="xs-title"><?php echo esc_html($table_value['table_title']); ?></h4>
                                    <?php endif; ?>
                                </div><!-- .pricing-header END -->

                                <div class="pricing-body">
                                    <?php if (!empty($table_value['table_sub_title'])) : ?>
                                        <p><?php echo hostinza_kses($table_value['table_sub_title']); ?></p>
                                    <?php endif; ?>

                                    <div class="pricing-price">
                                        <?php if (!empty($table_value['table_extra'])) : ?>
                                            <p><?php echo esc_html($table_value['table_extra']); ?></p>
                                        <?php endif; ?>
                                        <h2>
                                            <sup><?php echo esc_html($table_value['currency_icon']); ?></sup><?php echo esc_html($table_value['table_price']); ?>
                                        </h2>
                                        ...
                                        if (is_array($table_contents) && !empty($table_contents)) :
                                            foreach ($table_contents as $tab_text) :
                                                echo ' <h6 class="discount-price">';
                                                echo hostinza_kses(sprintf($tab_text, '<del>', '</del>'));
                                                echo '</h6>';
                                            endforeach;
                                        endif;
                                        ?>
                ...
                <div class="<?php echo esc_attr($grid);?> col-md-6">
                    <div class="xs-single-pricing <?php if ( $table_value['xs_featured_table'] == 'yes' ) echo 'active'; ?>">
                        ...
                        <?php if( !empty($table_value['button_text']) ) : ?>
                            <?php $btn_target = ( $table_value['button_url']['is_external']) ? '_blank' : '_self'; ?>
                            <div class="pricing-footer">
                                <a href="<?php echo esc_url($table_value['button_url']['url']); ?>" target="<?php echo esc_html( $btn_target ); ?>" class="btn btn-primary"><?php echo esc_html($table_value['button_text']); ?></a>
                            </div><!-- .pricing-footer END -->
                        <?php endif; ?>
                    </div><!-- .xs-single-pricing END -->
                </div>
