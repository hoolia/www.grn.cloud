 ...
 */
class Hostinza_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		?>
			...
			<p class="description description-wide nmi-design">
				<label for="edit-menu-item-design-<?php echo esc_attr( $item_id ); ?>">
					...
					<select id="edit-menu-item-design-<?php echo esc_attr( $item_id ); ?>" data-field="nmi-design" class="widefat" name="menu-item-design[<?php echo esc_attr( $item_id ); ?>]">
						<option value="default" <?php selected( $design, 'default', true); ?>><?php esc_html_e('Default', 'hostinza'); ?></option>
						<option value="full-width" <?php selected( $design, 'full-width', true); ?>><?php esc_html_e('Full width', 'hostinza'); ?></option>
						<option value="sized" <?php selected( $design, 'sized', true); ?>><?php esc_html_e('Set sizes', 'hostinza'); ?></option>
					</select>
				</label>
			</p>
			<p class="description description-thin nmi-width" style="display:none;">
				<label for="edit-menu-item-width-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Dropdown Width', 'hostinza'); ?><br>
			...
			<p class="description description-thin nmi-height" style="display:none;">
				<label for="edit-menu-item-height-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Dropdown Height', 'hostinza'); ?><br>
			...
			<p class="description description-wide nmi-block" style="display:none;">
				<label for="edit-menu-item-block-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('HTML Block for the dropdown', 'hostinza'); ?><br>
					<select id="edit-menu-item-block-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-block[<?php echo esc_attr( $item_id ); ?>]">
						<option value="" <?php selected( $block, '', true); ?>><?php esc_html_e('None', 'hostinza'); ?></option>
					...
					<a href="<?php echo admin_url( 'post.php?post=' . $block . '&action=edit' ); ?>" <?php if ( empty( $block ) ): ?>style="display:none;"<?php endif ?> class="edit-block-link" target="_blank"><?php esc_html_e( 'Edit this block with WPBakery Page Builder', 'hostinza' ); ?></a> |
					<a href="<?php echo admin_url( 'post-new.php?post_type=cms_block' ); ?>" class="add-block-link" target="_blank"><?php esc_html_e( 'Add new', 'hostinza' ); ?></a>
				</label>
			</p>
			<p class="description description-wide nmi-icon">
				<label for="edit-menu-item-height-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Icon name (from FontAwesome set)', 'hostinza'); ?><br>
			...
			<p class="description description-wide nmi-event">
				<label for="edit-menu-item-event-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Open on mouse event', 'hostinza'); ?><br>
					<select id="edit-menu-item-event-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-event[<?php echo esc_attr( $item_id ); ?>]">
						<option value="hover" <?php selected( $event, 'hover', true); ?>><?php esc_html_e('Hover', 'hostinza'); ?></option>
						<option value="click" <?php selected( $event, 'click', true); ?>><?php esc_html_e('Click', 'hostinza'); ?></option>
					</select>
				</label>
			</p>
			<p class="description description-wide nmi-label-text">
				<label for="edit-menu-item-label-text-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Label text', 'hostinza'); ?><br>
			...
			<p class="description description-wide nmi-label">
				<label for="edit-menu-item-label-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Label color', 'hostinza'); ?><br>
					<select id="edit-menu-item-label-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-label[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""></option>
						<option value="primary" <?php selected( $label, 'primary', true); ?>><?php esc_html_e('Primary Color', 'hostinza'); ?></option>
						<option value="secondary" <?php selected( $label, 'secondary', true); ?>><?php esc_html_e('Secondary', 'hostinza'); ?></option>
						<option value="red" <?php selected( $label, 'red', true); ?>><?php esc_html_e('Red', 'hostinza'); ?></option>
						<option value="green" <?php selected( $label, 'green', true); ?>><?php esc_html_e('Green', 'hostinza'); ?></option>
						<option value="blue" <?php selected( $label, 'blue', true); ?>><?php esc_html_e('Blue', 'hostinza'); ?></option>
						<option value="orange" <?php selected( $label, 'orange', true); ?>><?php esc_html_e('Orange', 'hostinza'); ?></option>
						<option value="grey" <?php selected( $label, 'grey', true); ?>><?php esc_html_e('Grey', 'hostinza'); ?></option>
						<option value="black" <?php selected( $label, 'black', true); ?>><?php esc_html_e('Black', 'hostinza'); ?></option>
						<option value="white" <?php selected( $label, 'white', true); ?>><?php esc_html_e('White', 'hostinza'); ?></option>
					</select>
				</label>
			</p>
			<p class="description description-wide nmi-color-scheme">
				<label for="edit-menu-item-colorscheme-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('Dropdown text color scheme', 'hostinza'); ?><br>
					<select id="edit-menu-item-colorscheme-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-colorscheme[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""></option>
						<option value="light" <?php selected( $color_scheme, 'light', true); ?>><?php esc_html_e('Light', 'hostinza'); ?></option>
						<option value="dark" <?php selected( $color_scheme, 'dark', true); ?>><?php esc_html_e('Dark', 'hostinza'); ?></option>
					</select>
				</label>
			</p>
			<p class="description description-wide nmi-opanchor">
				<label for="edit-menu-item-opanchor-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('One page anchor', 'hostinza'); ?><br>
					<select id="edit-menu-item-opanchor-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-opanchor[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""></option>
						<option value="enable" <?php selected( $opanchor, 'enable', true); ?>><?php esc_html_e('Enable', 'hostinza'); ?></option>
						<option value="disable" <?php selected( $opanchor, 'disable', true); ?>><?php esc_html_e('Disable', 'hostinza'); ?></option>
					</select>
					<span class="description"><?php esc_html_e('Enable this to use one page navigation menu. If enabled you need to set the link for this item to be like this: http://your_site.com/home_page/#anchor_id where anchor_id will be the ID of the ROW on your home page.', 'hostinza'); ?></span>
				</label>
			</p>
			<p class="description description-wide nmi-callbtn">
				<label for="edit-menu-item-callbtn-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e('"Call to action" button', 'hostinza'); ?><br>
					<select id="edit-menu-item-callbtn-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-callbtn[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""></option>
						<option value="enable" <?php selected( $callbtn, 'enable', true); ?>><?php esc_html_e('Enable', 'hostinza'); ?></option>
						<option value="disable" <?php selected( $callbtn, 'disable', true); ?>><?php esc_html_e('Disable', 'hostinza'); ?></option>
					</select>
				</label>
			</p>
		<?php
		...
		if ( has_post_thumbnail( $item_id ) ) {
			...
			$link_text = esc_html__( 'Change image', 'hostinza' );

			// For WP 3.5+, add 'remove' action link
			if ( version_compare( $wp_version, '3.5', '>=' ) ) {
				$ajax_nonce = wp_create_nonce( 'set_post_thumbnail-' . $item_id );
				$remove_link = ' | <a href="#" data-id="' . $item_id . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $item_id . ');return false;">' . esc_html__( 'Remove image', 'hostinza' ) . '</a>';
			}
		} else {
			$output .= '<div class="nmi-current-image nmi-div" style="display: none;"></div>';
			$link_text = esc_html__( 'Upload image', 'hostinza' );
		}
