 ...
 */
add_action( 'init', array( 'Hostinza_Nav_Menu_Item_Custom_Fields', 'setup' ) );
class Hostinza_Nav_Menu_Item_Custom_Fields {
    ...
    static function setup() {
        if ( !is_admin() )
            return;
        $new_fields = apply_filters( 'hostinza_nav_menu_item_additional_fields', array() );
        if ( empty($new_fields) )
            return;
        self::$options['fields'] = self::get_fields_schema( $new_fields );
        add_filter( 'wp_edit_nav_menu_walker', function () {
            return 'Hostinza_Walker_Nav_Menu_Edit';
        });
...
class Hostinza_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
    function start_el(&$output, $item, $depth=0, $args=array(), $id=0) {
        ...
        if ( $new_fields = Hostinza_Nav_Menu_Item_Custom_Fields::get_field( $item, $depth, $args ) ) {
...
// Somewhere in config...
add_filter( 'hostinza_nav_menu_item_additional_fields', 'hostinza_menu_item_additional_fields' );
function hostinza_menu_item_additional_fields( $fields ) {
    $fields['icon'] = array(
        'name' => 'icon',
        'label' => esc_html__('Menu Icon', 'hostinza'),
        'container_class' => '',
        'input_type' => 'text',
    );
