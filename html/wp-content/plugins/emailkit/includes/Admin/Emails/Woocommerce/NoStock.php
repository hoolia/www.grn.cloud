...
class NoStock {
	...
	public function stockNotification($product)
	{
		...
		if (isset($query->posts[0])) {
			...
			$pre_header = !empty($pre_header) ? $pre_header : esc_html__(' Stock Quantity Unavailable ', 'emailkit');
			...
			$subject = !empty($subject) ? $subject . ' - ' . $pre_header : $product->get_name() ." - " . esc_html__(' Out of Stock ', 'emailkit'). '-' . $pre_header;
