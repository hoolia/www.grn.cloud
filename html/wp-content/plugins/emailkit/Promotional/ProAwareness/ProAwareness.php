...
namespace EmailKit\Promotional\ProAwareness;

defined('ABSPATH') || exit;

if(!class_exists('\Wpmet\Libs\ProAwareness')) :
	...
	class ProAwareness
    {
		...
		public function set_page_grid($conf = []) {
			...
			if(!empty($conf['url'])) {
				...
				$this->grids[] = [
					'url'         => $conf['url'],
					'title'       => empty($conf['title']) ? esc_html__('Default Title', 'emailkit') : $conf['title'],
		...
		public function set_products( $product = [] ) {			
			$this->products[] = [
				...
				'title'       => empty( $product['title'] ) ? esc_html__( 'Default Title', 'emailkit' ) : $product['title'],
		...
		protected function prepare_grid_links() {
			...
			if(!empty($this->grids)) {
				...
				add_submenu_page($this->parent_menu_slug, $this->parent_menu_text, $this->parent_menu_text, 'manage_options', $this->text_domain . $this->menu_slug, [$this, 'generate_grids'], 2);
			}
		}
