	...
	public function get_plugin_status( $name ) {
		...
		if ( $this->check_installed_plugin( $name ) ) {
			if ( $this->check_activated_plugin( $name ) ) {
				$data['title']  = __( 'Activated', 'emailkit' );
				$data['status'] = "activated";
			} else {
				$data['title']          = __( 'Activate Now', 'emailkit' );
				$data['status']         = 'installed';
				$data['activation_url'] = $this->activation_url( $name );
			}
		} else {
			$data['title']            = __( 'Install Now', 'emailkit' );
					...
					<div class="attr-col-lg-4">
						<div class="wpmet-onboard-single-plugin">
							<label>
								...
								$plugin_status_title = isset( $plugin_data['title'] ) ? $plugin_data['title'] : esc_html__('Activate', 'emailkit');
								?>
								<div class="wpmet-apps-footer">
									...
									if( !empty($plugin_docs) ) :
										echo sprintf(
											'<a target="_blank" href="%1$s" class="">%2$s</a>',
											esc_url($plugin_docs),
											esc_html__('Read Docs', 'emailkit')
										);
									endif;
									?>
								</div>
