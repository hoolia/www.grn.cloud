 ...
 */
class Hostinza_recent_post extends WP_Widget {

	function __construct() {

		$widget_opt = array(
			'classname'		 => 'hostinza_widget',
			'description'	 => esc_html__('Recent Post With Thumbnail','hostinza')
		);

		parent::__construct( 'xs-recent-post', esc_html__( 'Hostinza Recent Post', 'hostinza' ), $widget_opt );
	}
		...
		<div class="widget-posts">
				...
				while ( $loop->have_posts() ):
					...
					<div class="widget-post media">
						<?php
							...
							echo '<a href="'.get_the_permalink().'"><img src="' . esc_url( $img ) . '" alt="' . esc_attr__('thumb','hostinza') . '"></a>';
							?>
			...
			<?php else: ?>
				<div class="nopost_message">
					<p><?php echo esc_html__( 'No Post Available', 'hostinza' ) ?></p>';
				</div>
			<?php endif; ?>  
			</div>
		<?php
	...
	function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_html__( 'Recent Posts', 'hostinza' );
		}
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php esc_html_e( 'Number Of Posts:', 'hostinza' ); ?></label>
